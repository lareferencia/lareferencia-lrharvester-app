  ...
  <!-- Publication Entity -->
    <xsl:template name="publication">
        <xsl:element name="entity">
            ...
            <xsl:text>Publication1</xsl:text>
        </xsl:attribute>
            <!-- semanticIdentifier -->
            <xsl:call-template name="semanticIdentifier">
                <xsl:with-param name="value"
                    select="/doc:metadata/doc:element[@name='datacite']/doc:element[@name='identifier']/doc:field[@name='value']/text()"/>
            </xsl:call-template>
...
<element name="relatedIdentifiers">
         <element name="relatedIdentifier">
            <field name="relatedIdentifierType">ISSN</field>
            <field name="value">0020-7136</field>
         </element>
         <element name="relatedIdentifier">
            <field name="relatedIdentifierType">DOI</field>
            <field name="value">10.1002/ijc.31935</field>
         </element>
      </element>  
-->
    ...
    <xsl:template
        ...
        <xsl:call-template name="field_value">
            ...
            <xsl:with-param name="value" select="format-dateTime(current-dateTime()
    ...
    <!-- date Accepted -->
    <xsl:template
        match="doc:element[@name='date']/doc:element[@name='Accepted']">
        <xsl:call-template name="field">
    ...
    <xsl:template match="doc:element[@name='datacite']/doc:element[@name='rights']" mode="Publication">
        ...
        <xsl:variable name="open_access">
            <xsl:choose>
                <!-- is open access ? -->
                <xsl:when test="doc:field[@name='rightsURI' and text()='http://purl.org/coar/access_right/c_abf2']">
    ...
    <!-- license -->
    <xsl:template match="doc:element[@name='oaire']/doc:element[@name='licenseCondition']" mode="Publication">
    ...
    <!-- Creator Entities (Person or Organization) -->
    <xsl:template match="doc:element[@name='creator']" mode="entity">
        ...
        <xsl:element name="entity">
            ...
            <!-- semanticIdentifier -->
            <xsl:variable name="semanticIdentifier_value">
                <xsl:choose>
                    <xsl:when test="$type='Organization'">
                        <xsl:value-of
                            select="doc:element[@name='nameIdentifier']/doc:field[@name='nameIdentifierScheme' and text()='isni']/../doc:field[@name='value']/text()"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:value-of
                            select="doc:element[@name='nameIdentifier']/doc:field[@name='nameIdentifierScheme' and text()='orcid']/../doc:field[@name='value']/text()"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:variable>
            <xsl:call-template name="semanticIdentifier">
                <xsl:with-param name="value" select="$semanticIdentifier_value"/>
            </xsl:call-template>
            <!-- name -->
            <xsl:choose>
                <xsl:when test="$type='Organization'">
                    <xsl:apply-templates select="doc:element[@name='creatorName']" mode="Organization"/>
                </xsl:when>
                <xsl:otherwise>
    ...
    <!-- Contributor Entities (Person, Services or Organization) -->
    <xsl:template match="doc:element[@name='contributor']" mode="entity">
        ...
        <xsl:variable name="type">
            <xsl:choose>
                <xsl:when test="$lc_nameType='organizational' and $lc_contributorType='hostinginstitution'">
        ...
        <xsl:element name="entity">
            ...
            <xsl:variable name="semanticIdentifier_value">
                <xsl:choose>
                    <xsl:when test="$type='Service'">
                        <xsl:value-of
                            select="doc:element[@name='nameIdentifier']/doc:field[@name='nameIdentifierScheme' and text()='e-mail']/../doc:field[@name='value']/text()"/>
                    </xsl:when>
                    <xsl:when test="$type='Organization'">
                        <xsl:value-of
                            select="doc:element[@name='nameIdentifier']/doc:field[@name='nameIdentifierScheme' and text()='isni']/../doc:field[@name='value']/text()"/>
            ...
            <xsl:choose>
                <xsl:when test="$type='Service'">
                    <xsl:apply-templates select="doc:element[@name='contributorName']" mode="Service"/>
                </xsl:when>
                <xsl:when test="$type='Organization'">
                    <xsl:apply-templates select="doc:element[@name='contributorName']" mode="Organization"/>
                </xsl:when>
                <xsl:otherwise>
    ...
    <xsl:template match="doc:element[@name='creator']" mode="relation">
        ...
        <xsl:element name="relation">
            ...
            <xsl:element name="member">
                ...
                <xsl:text>hasCreator</xsl:text>
            </xsl:attribute>
                <xsl:attribute name="fromRef">
                <xsl:text>Publication1</xsl:text>
            </xsl:attribute>
            </xsl:element>
            <xsl:element name="member">
                <xsl:attribute name="label">
                <xsl:text>isCreatorOf</xsl:text>
            </xsl:attribute>
                <xsl:attribute name="fromRef">
                <xsl:value-of select="concat('Creator',position())"/>
            </xsl:attribute>
    ...
    <xsl:template match="doc:element[@name='contributor']" mode="relation">
        ...
        <xsl:element name="relation">
            <xsl:attribute name="type">
                <xsl:choose>
                    <xsl:when
                test="$lc_nameType='organizational' and $lc_contributorType='hostinginstitution'">
            ...
            <xsl:element name="member">
                <xsl:attribute name="label">
                    <xsl:text>hasContributor</xsl:text>
                </xsl:attribute>
                <xsl:attribute name="fromRef">
                    <xsl:text>Publication1</xsl:text>
                </xsl:attribute>
            ...
            <xsl:element name="member">
                <xsl:attribute name="label">
                    <xsl:text>isContributorOf</xsl:text>
                </xsl:attribute>
                <xsl:attribute name="fromRef">
                    <xsl:value-of select="concat('Contributor',position())"/>
                </xsl:attribute>
    ...
    <xsl:template match="doc:element[@name='creator']" mode="relation_affiliation">
        ...
        <xsl:if test="./doc:element[@name='affiliation']">
            <xsl:element name="relation">
                ...
                <xsl:element name="member">
                    <xsl:attribute name="label">
                        <xsl:text>hasAffiliation</xsl:text>
                    </xsl:attribute>
                    <xsl:attribute name="fromRef">
                        <xsl:value-of select="concat('Creator',position())"/>
                    </xsl:attribute>
                ...
                <xsl:element name="member">
                    <xsl:attribute name="label">
                        <xsl:text>isAffiliatedOf</xsl:text>
                    </xsl:attribute>
                    <xsl:attribute name="fromRef">
                        <xsl:value-of select="concat('Affiliation',position())"/>
                    </xsl:attribute>
  ...
  <!-- Relation Funding -->
   ...
   <!-- field template -->
    <xsl:template name="field">
        ...
        <xsl:call-template name="field_value">
            <xsl:with-param name="name" select="$name"/>
            <xsl:with-param name="lang" select="$node/doc:field[@name='lang']/text()"/>
            <xsl:with-param name="value" select="$node/doc:field[@name='value']/text()"/>
        </xsl:call-template>
    </xsl:template>
